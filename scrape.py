...
a_elements = edusanjal.select('ul.relative.z-0.inline-flex.rounded-md.shadow-sm.-space-x-px.flex-wrap a.router-link-active.router-link-exact-active.bg-white.text-gray-500.hover\:bg-gray-50.inline-flex.relative.items-center.px-4.py-2.border.text-sm.font-medium')

if a_elements:
    last_page_element = a_elements[-1]
    last_page_number = last_page_element.text.strip()
    print(type(last_page_number))
else:
    print("Page number not found.")

while current_page<=int(last_page_number):
    current_website=f'{website}?page={current_page}'
    urls=[]
    ...
    for div in div_list:
        ...
        print(link)
        link = link[8:]
        total_url= current_website[:-8]+link
        print(total_url)
